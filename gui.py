...
tab1_layout = [
    [
        sg.Column([
            [sg.Radio("Encrypt with AES encryption", group_id='aes_rsa_radios',
                      key='aes_radio', default=True, enable_events=True)],
            [sg.Radio("Encrypt with RSA encryption", group_id='aes_rsa_radios',
                      key='rsa_radio', default=False, enable_events=True)],
            [sg.Radio("Do not encrypt", group_id='aes_rsa_radios',
                      key='no_encryption_radio', default=False, enable_events=True)],
            [sg.Checkbox("Hide data inside image", key='image_checkbox',
                         default=True, enable_events=True)],

            [sg.Text(text="Output folder", key='output_dir_title')],
            [sg.Column([[sg.Input(key='output_dir'), sg.FolderBrowse(
                key='output_dir_browse')]], key='output_dir_row')],
            [sg.Text(text='', key='output_dir_message', size=(40, 2))],

            [sg.Text(text="key", key='public_key_title')],
            [sg.Column([[sg.Input(key='public_key', disabled_readonly_background_color='black'), sg.FileBrowse(
                key='public_key_browse', file_types=(('PEM Files', '*.pem'), ))]], key='public_key_row')],
            [sg.Text(text='', key='public_key_message', size=(40, 2))],

            [sg.Text(text="Image file", key='image_file_title')],
            [sg.Column([[sg.Input(key='image_file', disabled_readonly_background_color='black'), sg.FileBrowse(
                key='image_file_browse', file_types=(('PNG Files', '*.png'), ))]], key='image_file_row')],
            [sg.Text(text='', key='image_file_message', size=(40, 2))],
            [sg.Text(text="This Project is developed by Sushant,Yashraj And Ritika")]
        ], vertical_alignment='top'),
        sg.VerticalSeparator(),
        sg.Column([
            [
                sg.Radio("Text", 'text_file_radios', key='text_radio',
                         default=True, enable_events=True),
                sg.Radio("File", 'text_file_radios', key='file_radio',
                         default=False, enable_events=True)
            ],
            [sg.Text(text="File", key='file_title')],
            [sg.Column([[sg.Input(key='file', disabled_readonly_background_color='black',
                       disabled=True), sg.FileBrowse(key='file_browse', disabled=True)]], key='file_row')],
            [sg.Text(text='', key='encrypt_file_text', size=(40, 2))],

            [sg.Text(text="Text", key='text_title')],
            [sg.Multiline(default_text='Your message here',
                          size=(40, 15), key='message')],
...
tab2_layout = [
    [
        sg.Column([
            ...
            [sg.Column([[sg.Input(key='encrypted_file'), sg.FileBrowse(
                key='encrypted_file_browse')]], key='encrypted_file_row')],
            [sg.Text(text='', key='encrypted_file_message', size=(40, 2))],

            [sg.Text(text="key", key='private_key_title')],
            [sg.Column([[sg.Input(key='private_key'), sg.FileBrowse(
                key='private_key_browse', file_types=(('PEM Files', '*.pem'), ))]], key='private_key_row')],
...
tab3_layout = [
    [
        sg.Column([
            [sg.Text("Key size")],
            [sg.Combo(('2048', '3072', '4096'),
                      default_value='3072', key='key_size')],
            [sg.Text("Output folder")],
            [sg.Column([[sg.Input(key='keys_dir'), sg.FolderBrowse(
                key='keys_dir_browse')]], key='keys_dir_row')],
...
layout = [
    [
        sg.TabGroup([
            [sg.Tab("Encrypt", tab1_layout), sg.Tab(
                "Decrypt", tab2_layout), sg.Tab("Keys", tab3_layout)]
        ])
    ]
]
...
window = sg.Window(
    "SECURE DATA ENCRYPTION AND DECRYPTION USING CRYPTO-STEGO", layout)
while True:
    window.refresh()
    ...
    if event == sg.WIN_CLOSED:
        ...
        if not os.path.exists(values['keys_dir']):
            window['keys_dir_message'].update(
                "Invalid folder", text_color='red')
        else:
            try:
                public_key, private_key = crypto.generate_key_pair(
                    int(values['key_size']))
                save_file(
                    f"{values['keys_dir']}/public_{values['key_size']}.pem", public_key)
                save_file(
                    f"{values['keys_dir']}/private_{values['key_size']}.pem", private_key)
                window['keys_dir_message'].update(
                    "Successfully generated key pair", text_color='green')
            except Exception as exception:
                window['keys_dir_message'].update(
                    f"Unable to generate key pair\n{exception}", text_color='red')
        ...
        try:
            if values['rsa_radio']:
                if not values['image_checkbox']:
                    if os.path.exists(values['output_dir']) and os.path.exists(values['public_key']):
                        ...
                        if values['text_radio']:
                            ...
                            encrypted = crypto.encrypt_rsa(
                                values['message'], values['public_key'], 'rsa_text')
                            save_file(output_path, encrypted)
                            window['encrypt_message_text'].update(
                                "Successfully encrypted", text_color='green')
                            ...
                            encrypted = crypto.encrypt_rsa(
                                file_data, values['public_key'], f"rsa_file:{os.path.basename(values['file'])}")
                            save_file(output_path, encrypted)
                            window['encrypt_message_text'].update(
                                "Successfully encrypted", text_color='green')
                    ...
                    if os.path.exists(values['output_dir']) and os.path.exists(values['public_key']) and os.path.exists(values['image_file']):
                        ...
                        if values['text_radio']:
                            encrypted = crypto.encrypt_rsa(
                                values['message'], values['public_key'], 'rsa_text')
                            stego.encode(
                                encrypted, values['image_file'], f"{values['output_dir']}/encrypted.png")
                            window['encrypt_message_text'].update(
                                "Successfully encrypted", text_color='green')
                    ...
                    if os.path.exists(values['output_dir']) and os.path.exists(values['public_key']):
                        ...
                        if values['text_radio']:
                            ...
                            encrypted = crypto.encrypt_aes(
                                values['message'], values['public_key'], 'aes_text')
                            save_file(output_path, encrypted)
                            window['encrypt_message_text'].update(
                                "Successfully encrypted", text_color='green')
                            ...
                            output_path = f"{values['output_dir']}/{os.path.basename(values['file'])}.encrypted"
                            encrypted = crypto.encrypt_aes(
                                file_data, values['public_key'], f"aes_file:{os.path.basename(values['file'])}")
                            save_file(output_path, encrypted)
                            window['encrypt_message_text'].update(
                                "Successfully encrypted", text_color='green')
                ...
                if os.path.exists(values['output_dir']) and os.path.exists(values['image_file']):
                    ...
                    if values['text_radio']:
                        stego.encode(
                            'plain_text\u2028' + values['message'], values['image_file'], f"{values['output_dir']}/encrypted.png")
                        window['encrypt_message_text'].update(
                            "Successfully encrypted", text_color='green')
                        ...
                        with open(values['file'], 'rb') as input_file:
                            file_data = input_file.read()
                        stego.encode(f"plain_file:{os.path.basename(values['file'])}\u2028".encode(
                            'utf-8') + file_data, values['image_file'], f"{values['output_dir']}/encrypted.png")
                        window['encrypt_message_text'].update(
                            "Successfully encrypted", text_color='green')
        ...
        try:
            ...
            if values['encrypted_file'].split('.')[-1].lower() == 'png':
                ...
                if encrypted.split(sep)[0] == b'plain_text':
                    window['decrypted_message'].update(
                        encrypted.split(sep)[1].decode('utf-8'))
                    window['decrypt_message'].update(
                        'Successfully decrypted file', text_color='green')
                    continue

                elif encrypted.split(sep)[0].startswith(b'plain_file:'):
                    filename = encrypted.split(sep)[0].split(b':')[
                        1].decode('utf-8')
                    decrypted = encrypted.split(sep)[1]
                    filename = save_file(
                        f"{values['output_decrypt_dir']}/{filename}", decrypted)
                    try:
                        window['decrypted_message'].update(
                            f"Saved decrypted data to file '{filename}'. Contents:\n\n{decrypted.decode('utf-8')}")
                    except Exception:
                        window['decrypted_message'].update(
                            f"Saved decrypted data to file '{filename}'.")
                    window['decrypt_message'].update(
                        "Successfully decrypted file", text_color='green')
                    continue
            ...
            if header == b'rsa_text':
                header, decrypted = crypto.decrypt_rsa(
                    encrypted, values['private_key'])
                window['decrypted_message'].update(decrypted.decode('utf-8'))
                window['decrypt_message'].update(
                    "Successfully decrypted file", text_color='green')

            elif header == b'aes_text':
                header, decrypted = crypto.decrypt_aes(
                    encrypted, values['private_key'])
                window['decrypted_message'].update(decrypted.decode('utf-8'))
                window['decrypt_message'].update(
                    "Successfully decrypted file", text_color='green')
                ...
                window['output_decrypt_dir_message'].update(
                    "Invalid folder", text_color='red')
